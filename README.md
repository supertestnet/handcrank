...
The withdrawal transactions are a set of several thousand nearly identical bitcoin transactions. The first one is special, its mega signature can only be used to send the depositor’s money into an initial “withdrawal address.” Every other withdrawal transaction, if broadcasted, does one of two things.

If its signature is not passed in the witness data and 13,150 blocks (3 months) have passed since the money entered this address, the money becomes spendable by anyone. If the signature *is* passed in the witness data and 1 block has passed since the money entered this address, the money goes straight back into the current address, restarting the countdown from 13,150 and thus delaying the withdrawal. If a miner opts not to broadcast a withdrawal transaction, the transaction will continue to progress through its current timelock one block at a time.

The miners signed 13,150 such transactions so if miners delay the withdrawal once per block, a user's funds might be locked up for as many as 26,300 blocks, or about 6 months, before the money becomes spendable by anyone. But if miners do not hinder its progress, withdrawals can be processed in only 13,150 blocks i.e. 3 months.

## 7 & 8. Any bitcoin miner can check the new person’s claim and start sending them Alice’s deposit, and during the delay, any miner who rejects the claim can extend the delay up to 6 months
