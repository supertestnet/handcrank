...
Use a multisig with 51% of miners to emulate a covenant and use that covenant to make a hashrate escrow (and thus a drivechain). This is a downgrade compared to how bip300 does it (because with bip300 all bitcoin nodes enforce the covenant, not just miners) but it should be fine because drivechain assumes 51% of miners are honest anyway.
